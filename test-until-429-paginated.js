 ...
 * Object 1, 10 records per page, incrementing page number each iteration
 */

import { FireberryClient } from './dist/index.js';

async function queryUntil429() {
  console.log('🧪 Querying Until 429 Error (Paginated)\n');
  ...
  console.log('   Page Number: Incrementing on each query');
  console.log('   Retry on 429: DISABLED (to detect 429 immediately)\n');

  try {
    while (true) {
      queryCount++;
      const pageNumber = queryCount; // Use query count as page number
      ...
      try {
        ...
        if (queryCount % 100 === 0) {
          console.log(`✅ Query ${queryCount} (page ${pageNumber}): ${result.records.length} records | Total: ${totalRecords} | ${elapsed}s | ${rate} q/s`);
        }
        ...
        console.error(`\n${'='.repeat(70)}`);
        console.error(`❌ ERROR on query ${queryCount} (page ${pageNumber}) at ${elapsed}s`);
